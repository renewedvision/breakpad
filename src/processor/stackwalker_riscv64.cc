...
StackFrameRISCV64* StackwalkerRISCV64::GetCallerByCFIFrameInfo(
  ...
  StackFrameRISCV64* last_frame = static_cast<StackFrameRISCV64*>(frames.back());
  ...
  } else if (last_frame->context_validity & StackFrameRISCV64::CONTEXT_VALID_S0) {
    // Since the register is callee-saves, assume the callee has not yet chenged it
    frame->context_validity |= StackFrameRISCV64::CONTEXT_VALID_S0;
  ...
  } else if (last_frame->context_validity & StackFrameRISCV64::CONTEXT_VALID_S1) {
    // Since the register is callee-saves, assume the callee has not yet chenged it
    frame->context_validity |= StackFrameRISCV64::CONTEXT_VALID_S1;
  ...
  } else if (last_frame->context_validity & StackFrameRISCV64::CONTEXT_VALID_S2) {
    // Since the register is callee-saves, assume the callee has not yet chenged it
    frame->context_validity |= StackFrameRISCV64::CONTEXT_VALID_S2;
  ...
  } else if (last_frame->context_validity & StackFrameRISCV64::CONTEXT_VALID_S3) {
    // Since the register is callee-saves, assume the callee has not yet chenged it
    frame->context_validity |= StackFrameRISCV64::CONTEXT_VALID_S3;
  ...
  } else if (last_frame->context_validity & StackFrameRISCV64::CONTEXT_VALID_S4) {
    // Since the register is callee-saves, assume the callee has not yet chenged it
    frame->context_validity |= StackFrameRISCV64::CONTEXT_VALID_S4;
  ...
  } else if (last_frame->context_validity & StackFrameRISCV64::CONTEXT_VALID_S5) {
    // Since the register is callee-saves, assume the callee has not yet chenged it
    frame->context_validity |= StackFrameRISCV64::CONTEXT_VALID_S5;
  ...
  } else if (last_frame->context_validity & StackFrameRISCV64::CONTEXT_VALID_S6) {
    // Since the register is callee-saves, assume the callee has not yet chenged it
    frame->context_validity |= StackFrameRISCV64::CONTEXT_VALID_S6;
  ...
  } else if (last_frame->context_validity & StackFrameRISCV64::CONTEXT_VALID_S7) {
    // Since the register is callee-saves, assume the callee has not yet chenged it
    frame->context_validity |= StackFrameRISCV64::CONTEXT_VALID_S7;
  ...
  } else if (last_frame->context_validity & StackFrameRISCV64::CONTEXT_VALID_S8) {
    // Since the register is callee-saves, assume the callee has not yet chenged it
    frame->context_validity |= StackFrameRISCV64::CONTEXT_VALID_S8;
  ...
  } else if (last_frame->context_validity & StackFrameRISCV64::CONTEXT_VALID_S9) {
    // Since the register is callee-saves, assume the callee has not yet chenged it
    frame->context_validity |= StackFrameRISCV64::CONTEXT_VALID_S9;
  ...
  } else if (last_frame->context_validity & StackFrameRISCV64::CONTEXT_VALID_S10) {
    // Since the register is callee-saves, assume the callee has not yet chenged it
    frame->context_validity |= StackFrameRISCV64::CONTEXT_VALID_S10;
  ...
  } else if (last_frame->context_validity & StackFrameRISCV64::CONTEXT_VALID_S11) {
    // Since the register is callee-saves, assume the callee has not yet chenged it
    frame->context_validity |= StackFrameRISCV64::CONTEXT_VALID_S11;
...
StackFrame* StackwalkerRISCV64::GetCallerFrame(const CallStack* stack,
  ...
  if (TerminateWalk(frame->context.pc, frame->context.sp, last_frame->context.sp,
                    last_frame->trust == StackFrame::FRAME_TRUST_CONTEXT)) {
    return NULL;
  }
