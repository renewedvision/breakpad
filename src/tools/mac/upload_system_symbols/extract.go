...
func ExtractCache(format ArchiveFormat, archive string, destination string, verbose bool) error {
	var e extractor
	switch format {
	case IPSW:
		e = &ipswExtractor{}
...
func (e *installAssistantExtractor) Extract(installerPath string, destination string, verbose bool) (retErr error) {
	e.verbose = verbose
	...
	defer func() {
		if err := unmountDMG(dmgMountpoint); err != nil {
			retErr = errors.Join(retErr, fmt.Errorf("couldn't unmount disk image at %s: %v", dmgMountpoint, err))
		}
	}()
	...
	return e.extractCachesFromZips(zips, destination, hasCryptexes)
}
...
func (e *installAssistantExtractor) extractCachesFromCryptexes(cryptexesPath string, destination string) (retErr error) {
	...
	for _, f := range files {
		...
		defer func() {
			if err := unmountDMG(cryptexMountpoint); err != nil {
				retErr = errors.Join(retErr, fmt.Errorf("couldn't unmount disk image at %s: %v", cryptexMountpoint, err))
			}
		}()
...
func (e *ipswExtractor) Extract(ipswPath string, destination string, verbose bool) (retErr error) {
	e.verbose = verbose
	...
	defer func() {
		if err := unmountDMG(system); err != nil {
			retErr = errors.Join(retErr, fmt.Errorf("couldn't unmount disk image at %s: %v", system, err))
		}
	}()
