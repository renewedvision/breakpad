...
The _inline_nest_level_ field is a decimal number that describes in which level
this record represents. In the example below, first and third `INLINE` records
have _inline_nest_level_ 0, which means they are inlined inside the function
described by the `FUNC` record. The second `INLINE` record has
_inline_nest_level 1 means that it's inlined at the inline function described by
first `INLINE` record.
```
FUNC ...
INLINE 0 ...
...
The _origin_num_ field refers to an `INLINE_ORIGIN` record that gives the name
...
pair, since inline functions could have discontinuous addres ranges. The ranges
of a `INLINE` record are always inside the ranges described by its parent
(a `FUNC` record or an `INLINE` record).
